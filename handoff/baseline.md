...
>
> v2 更新：2026-07-16（新增 Baseline 假设 11——"中途误触发"，对应用户反馈"我发现在中途也会触发"）

---

## v2 新增假设

### Baseline 假设 11 — AI 在任务活跃推进中误触发 handoff

**v1 失败模式**：v1 把"AI 主动触发"放在 Step 0 的小节，且触发条件松（≥ 5 文件 + ≥ 2 commit + dirty + 长会话）——AI 看到任务有进展就问"要不要交接"，打断用户正常推进。

**用户原话**："我发现在中途也会触发，这个能优化吗？能改成主动触发式的吗？"

**用户实际想要的触发模式**：
- **默认主动**：仅在两个临界点同时满足时建议（上下文即将耗尽 + 任务接近完成）
- **中信号词不触发**："暂停一下"/"等下回来"/"brb" 这类 5 分钟休息词不触发
- **活跃推进中不触发**：用户刚发指令 / AI 刚答完问题 / 最近 commit ≤ 5 分钟不触发

**v2 skill 对应规则**：Step 0 重写为"触发模式 A（主动默认）+ 触发模式 B（强信号兜底）"；中信号词显式排除表；活跃推进 3 条约束（commit ≤ 5 分钟不触发 / 用户刚发指令不触发 / AI 刚答完不触发）。

**evals 覆盖**：eval-3 改名为 `proactive-only-at-critical-points`，期望改为"不主动建议"（任务活跃推进中）；eval-6 `mid-session-pause-must-not-trigger` 验证"暂停一下" 不触发。

---

## v1 baseline（10 条，保留作为设计依据）
...
| **v2** Step 0 触发模式 A（两个临界点同时满足）+ 模式 B（强信号兜底）+ 中信号词排除表 + 活跃推进 3 约束 | **Baseline 假设 11** |
...
- **Baseline 假设 11** 实测命中——用户反馈"中途也会触发"，证明 v1 触发条件过松。
- **v2 改进**：触发模式 A 收紧为两个临界点同时满足；触发模式 B 显式排除中信号词；活跃推进 3 条硬约束。
- **下一次 eval 重点**：测试"用户刚 commit 完不到 5 分钟 / 用户在 todo 列表上刚打勾"等场景下 AI 是否仍会触发。

下次有人发现 baseline 假设 12+，请追加到本文件。
